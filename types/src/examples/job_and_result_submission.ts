...
;(async () => {
  ...
  const ALICE_ROLE_SEED = new Uint8Array(
    Buffer.from(
  ...
  const BOB_ROLE_SEED = new Uint8Array(
    Buffer.from(
  ...
  await new Promise(async resolve => {
    const unsub = await creatingProfileTx.signAndSend(
      ALICE,
      async ({ events = [], status }) => {
        if (status.isInBlock) {
          console.log(
  ...
  await new Promise(async resolve => {
    const unsub = await submittingJobTx.signAndSend(
      ALICE,
      async ({ events = [], status }) => {
        if (status.isInBlock) {
          console.log(
  ...
  const submittingJobResultTx = api.tx.jobs.submitJobResult(
    ...
    jobId,
    {
      DKGPhaseOne: {
        ...
        signatures: [
          u8aToHex(Uint8Array.from(signature1Array)),
          u8aToHex(Uint8Array.from(signature2Array)),
        ],
        threshold: 1,
        signatureScheme: {
          Ecdsa: {},
  ...
  await new Promise(async resolve => {
    const unsub = await submittingJobResultTx.signAndSend(
      ALICE,
      async ({ events = [], status }) => {
        if (status.isInBlock) {
          console.log(
